...
const GridTreePage3 = () => {
    ...
    const svgElements = Array.from({ length: 72 }, (_, index) => (
        <div key={index} data-tooltip-target={`tooltip`} className='transition ease-in-out p-4 rounded-lg hover:-translate-y-1 hover:scale-110 duration-300'>
            <svg width="72" height="72" viewBox="0 0 72 72" fill="none" xmlns="http://www.w3.org/2000/svg" className={`fill-tree-200 opacity-50 hover:opacity-100`}>
            ...
            <div id={`tooltip`} role="tooltip" className={`absolute z-10 w-60 invisible inline-block px-3 py-2 text-md text-white text-center font-medium transition-opacity duration-300 bg-tree-200 shadow-sm opacity-0 tooltip`}>
